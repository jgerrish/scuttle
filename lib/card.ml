...
module Card =
  struct
    ...
    type card_rank = Ace | King | Queen | Jack
                     | Two | Three | Four | Five | Six | Seven | Eight | Nine | Ten [@@deriving compare, sexp]
    ...
    let%test_unit "test_card" =
      let card = { suit = Clubs; rank = Three } in
      [%test_eq: card_suit] card.suit Clubs
  end
